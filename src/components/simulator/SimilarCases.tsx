...
function buildLookupUrl(caso: CaseData) {
  if (caso.url_fonte) {
    return {
      href: caso.url_fonte,
      label: "Abrir fonte",
  ...
  const query = [caso.numero_processo, caso.tribunal, caso.titulo]
    .filter(Boolean)
    .join(" ");
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-lg font-bold text-white">
                Leitura comparativa com a base publica
              </h4>
      ...
      {cases.length > 0 && (
        <div className="surface-card p-6">
          ...
          <p className="mb-5 text-sm text-slate-400">
            {subtitle ??
              `Encontramos ${cases.length} casos trabalhistas com proximidade relevante ao relato informado.`}
          </p>
          ...
          <div className="space-y-4">
            {cases.map((caso) => {
              const lookup = buildLookupUrl(caso);
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    {lookup ? (
                      <a
                        ...
                        className="inline-flex items-center gap-2 text-xs font-semibold text-sky-300 hover:text-white"
                      >
                        {caso.url_fonte ? (
                      ...
                      <span className="text-xs text-slate-500">
                        Fonte do caso nao cadastrada na base.
                      </span>
                    )}
                    {!caso.url_fonte && caso.numero_processo ? (
                      <span className="text-[11px] text-slate-500">
                        Fonte direta indisponivel. O link abre uma pesquisa publica pelo numero do processo.
                      </span>
                    ) : null}
                  </div>
                </div>
          ...
          <p className="mt-4 text-center text-[11px] text-slate-500">
            ...
            valor e tempo seguem dependendo de prova e estrategia. Quando a
            fonte direta nao estiver cadastrada, o link abre uma pesquisa pelo
            numero do processo.
          </p>
        </div>
      )}
